...
import src.WhiteTeam;

import java.util.ArrayList;

public class WhitePawnMove implements MoveType {
    ...
    public ArrayList<String> getValidMoves(String position) {
        ...
        if (pieceMoveForwardOnce == null && pieceMoveForwardOnce.getInitPosY() == 1) {
        ...
        if (row - 1 >= 0 && col - 1 >= 0) {
            ...
            if (pieceMoveDiagonalLeft != null && pieceMoveDiagonalLeft.getColor() == 'B') {
                validMoves.add(moveDiagonalLeft);
            }
        }
        if (row -1 >= 0 && col + 1 < Chessboard.DIMENSION) {
            pieceMoveDiagonalRight = Chessboard.board[col + 1][row - 1];
            if (pieceMoveDiagonalRight != null && pieceMoveDiagonalRight.getColor() == 'B') {
